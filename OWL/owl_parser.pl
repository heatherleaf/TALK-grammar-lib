...
owl_report :-
	ontology(O,OA,CL,SL,PL,APL,DIL,SIL,IL,DSL,ESL,BNL),
...
collapse_ns(Full_URL, NS_URL) :-
	nonvar(Full_URL), 
	not(Full_URL = literal(_)),
	uri_split(Full_URL,Long_NS,Term, '#'), 
	concat(Long_NS,'#',Long_NS1),
	rdf_db:ns(Short_NS,Long_NS1),!,
	concat_atom([Short_NS,':',Term],NS_URL).
