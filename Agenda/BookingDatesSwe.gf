...
date__weekday w = {s = variants{("på" ++ w.s) ; w.s}};
date__weekday_day w d = {s = (variants {("på" ++ w.s);w.s}) ++ d.s};
date__weekday_day_month w d m = {s = (variants {("på" ++ w.s);w.s}) ++  d.s ++  m.s};


-- date__year y = y;
-- date__month_year m y = {s = m.s ++ y.s};
-- date__day_month_year d m y = {s = d.s ++ m.s ++ y.s};
-- date__weekday_day_month_year w d m y = {s = (variants {("på" ++ w.s);w.s}) ++ d.s ++ m.s ++ y.s};
