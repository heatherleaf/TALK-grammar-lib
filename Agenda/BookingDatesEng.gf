...
that_date = sing_NP ( variants {["that day"];["that date"];["on that day"];["on that date"]});

-- Since Date information is generated as classes

{-
mkDate s = mkNP s.s s.s Sg Masc;
mkMonth s = mkNP s.s s.s Sg Masc;
...
date__weekday w = {s = variants {("on" ++ w.s) ; w.s}};
date__weekday_day w d = {s = (variants {("on" ++ w.s) ; w.s}) ++ d.s};
date__weekday_day_month w d m = {s = (variants {("on" ++ w.s) ; w.s}) ++  d.s ++ "of" ++  m.s};


-- date__year y = y;
-- date__month_year m y = {s = m.s ++ y.s};
-- date__day_month_year d m y = {s = d.s ++ m.s ++ y.s};
-- date__weekday_day_month_year w d m y = {s = (variants {("on" ++ w.s) ; w.s}) ++ d.s ++ "of" ++  m.s ++ y.s};
